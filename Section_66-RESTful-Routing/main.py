...
class Cafe(db.Model):
    ...
    def to_dict(self):
        # Dictionary Comprehension to loop through each column in the data record
...
def get_all_cafes():
    all_cafes = db.session.query(Cafe).all()
    json_data = jsonify(cafe=[cafe.to_dict() for cafe in all_cafes])
    return json_data
...
def search_cafes():
    ...
    if cafes:
        json_data = jsonify(cafe=[cafe.to_dict() for cafe in cafes])
        # json_data = jsonify(cafe=cafes.to_dict())
        return json_data
    else:
        return error
